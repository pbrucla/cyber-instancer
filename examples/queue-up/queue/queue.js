...
async function main() {
  ...
  const sequelize = new Sequelize(
    `postgres://${process.env.POSTGRES_USER}:${process.env.POSTGRES_PASSWORD}@db`,
    { logging: true }
  );
  const app = express();
    ...
    if (Date.now() < process.env.startTime) {
      ...
      const served = (
        await sequelize.query(
          'select * from public."Queues" where position = (select min(position) from public."Queues" where served = false);',
          {
            model: Queue,
            mapToModel: true,
          }
  ...
  app.get("/", async function (req, res) {
    ...
    if (uuid !== undefined) {
      ...
      if (user === null) {
        ...
        res.render("flagredirect", {
          uuid: uuid,
          flagserverurl: process.env.FLAG_SERVER_URL,
        });
        return;
      } else {
    ...
    if (uuid === undefined) {
      ...
      await sequelize.query(
        `insert into public."Queues" (uuid, position) values ('${uuid}', (select max(position) + 1 from "Queues"));`
      );
    ...
    res.render("index", {
      title: `Queue Pos: ${relPos}`,
      position: relPos,
      time: forHumans(relPos * 5 * 60),
    });
  });
